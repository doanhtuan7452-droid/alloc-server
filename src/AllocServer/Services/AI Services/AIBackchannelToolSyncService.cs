        ...
        private async Task SyncToolsWithRetryAsync(CancellationToken cancellationToken)
        {
            ...
            var registerPayload = new RegisterToolsPayload
            {
                Tools = new List<DynamicToolDto>
                {
                    ...
                    {
                        Name = "create_project",
                        Description = _promptLoader.LoadPrompt("create_project",
                                      "Tạo một dự án mới trong Workspace được chỉ định. LLM bắt buộc phải điền đầy đủ các tham số được yêu cầu. Hợp đồng quy chuẩn bắt buộc:\n" +
                    ...
                    {
                        Name = "create_task",
                        Description = _promptLoader.LoadPrompt("create_task",
                                      "Tạo một nhiệm vụ (task) mới trong dự án được chỉ định. LLM bắt buộc điền các tham số được yêu cầu. Hợp đồng quy chuẩn bắt buộc:\n" +
                    ...
                    new DynamicToolDto
                    {
                        ...
                        Description = _promptLoader.LoadPrompt("get_project_info",
                                      "Lấy thông tin tổng quan của một dự án bao gồm thời gian thực hiện, ngân sách, số lượng task, số lượng rủi ro đang mở, điểm rủi ro trung bình và tổng chi phí tối ưu hóa qua View ProjectRiskFeatures. Lưu ý: Trường tasksByStatus có thể trả về một đối tượng rỗng '{}' nếu dự án mới được khởi tạo và chưa có công việc nào. Trong trường hợp này, LLM nên giải thích tự nhiên cho người dùng rằng dự án hiện chưa được phân bổ công việc nào. Yêu cầu truyền projectId và userId. LƯU Ý QUAN TRỌNG: Nếu người dùng chỉ nhắc đến TÊN dự án mà chưa có ID dự án (projectId), AI Agent PHẢI tự động gọi tool 'get_workspace_projects' trước để tra cứu tìm ra projectId từ tên dự án đó, tuyệt đối KHÔNG được hỏi người dùng cung cấp projectId."),
                    ...
                    new DynamicToolDto
                    {
                        ...
                        Description = _promptLoader.LoadPrompt("get_employee_list",
                                      "Lấy danh sách các nhân sự đang hoạt động trong Workspace của dự án. Không chứa các trường thông tin nhạy cảm. Yêu cầu truyền userId và một trong hai tham số: workspaceId hoặc projectId. LƯU Ý QUAN TRỌNG: Nếu người dùng nhắc đến tên dự án nhưng chưa có projectId, AI Agent PHẢI tự động gọi tool 'get_workspace_projects' trước để tra cứu tìm ra projectId từ tên dự án đó, tuyệt đối KHÔNG được hỏi người dùng cung cấp projectId."),
                    ...
                    new DynamicToolDto
                    {
                        ...
                        Description = _promptLoader.LoadPrompt("get_workspace_projects",
                                      "Lấy danh sách các dự án trong một Workspace hiện tại của bạn, bao gồm thông tin chi tiết (tên, ngày bắt đầu/kết thúc, ngân sách, doanh thu, phương pháp luận) và các chỉ số phân tích rủi ro/độ phức tạp tổng hợp từ View ProjectRiskFeatures. Hỗ trợ lọc theo trạng thái dự án, sắp xếp, và phân trang kết quả. LƯU Ý QUAN TRỌNG: Đây là công cụ duy nhất dùng để tra cứu và tìm projectId từ tên dự án. AI Agent được khuyến khích tự động gọi công cụ này trước để lấy projectId khi người dùng chỉ cung cấp tên dự án nhằm thực hiện các thao tác tiếp theo."),
